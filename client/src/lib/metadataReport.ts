...
import { CleanCopyFormat, ImageInspection, outputDimensionsForResize, ResizeOptions } from "@/lib/image";

export const SAFE_CSV_FIELDS = ["format", "mime_type", "dimensions", "megapixels", "file_size_category", "exif", "location_signal", "icc_profile", "text_comments", "xmp_packet", "offered", "selected_format", "jpeg_quality", "output_dimensions", "estimated_size_category", "report_excludes"] as const;
export type SafeCsvField = (typeof SAFE_CSV_FIELDS)[number];
export const DEFAULT_CSV_FIELDS: SafeCsvField[] = [...SAFE_CSV_FIELDS];

export type ReportOptions = { cleanFormat: CleanCopyFormat; jpegQuality: number; resizeOptions?: ResizeOptions | null; estimatedBytes: number | null; cleanCopyOffered: boolean };
...
export function metadataReportRows(inspection: ImageInspection, options: ReportOptions): string[][] {
  ...
  return [
    ...
    ["clean_copy", "offered", options.cleanCopyOffered ? "yes" : "no", "not-applicable"], ["clean_copy", "selected_format", options.cleanFormat, "not-applicable"], ["clean_copy", "jpeg_quality", options.cleanFormat === "jpeg" ? String(options.jpegQuality) : "not-applicable", "not-applicable"], ["clean_copy", "output_dimensions", `${outputDimensions.width}x${outputDimensions.height}`, outputDimensions.resized ? "resized-locally" : "original-dimensions"], ["clean_copy", "estimated_size_category", sizeCategory(options.estimatedBytes), "not-applicable"],
