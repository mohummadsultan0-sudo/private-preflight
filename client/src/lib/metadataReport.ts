...
export const SAFE_CSV_FIELDS = ["format", "mime_type", "dimensions", "megapixels", "file_size_category", "exif", "location_signal", "icc_profile", "text_comments", "xmp_packet", "offered", "selected_format", "jpeg_quality", "output_dimensions", "output_name", "orientation_correction", "estimated_size_category", "report_excludes"] as const;
export type SafeCsvField = (typeof SAFE_CSV_FIELDS)[number];
export const DEFAULT_CSV_FIELDS: SafeCsvField[] = [...SAFE_CSV_FIELDS];

export type ReportOptions = { cleanFormat: CleanCopyFormat; jpegQuality: number; resizeOptions?: ResizeOptions | null; estimatedBytes: number | null; cleanCopyOffered: boolean; anonymizeOutputName?: boolean };
